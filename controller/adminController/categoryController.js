...
const category = async (req, res) => {
  try {
    ...
    const category = await categorySchema
      .find({ categoryName: { $regex: categorySearch, $options: "i" } })
      .skip(skip)
      .limit(dataPerPage)
      .sort({ createdAt: -1 });
  ...
  try {
    ...
    if (checkCategory === null) {
      await categorySchema
        ...
        .catch((err) => {
          console.log(
            `Error occurred during adding new category to the collection ${err}`
          );
        });
...
const editCategory = async (req, res) => {
  try {
    ...
    if (category != null) {
      ...
      req.flash(
        "errorMessage",
        "An unexpected error occurred while editing the category. Please attempt the operation again."
      );
      res.redirect("/admin/category");
    }
  } catch (err) {
...
const editCategoryPost = async (req, res) => {
  try {
    ...
    if (checkCategory == null) {
      ...
      req.flash(
        "errorMessage",
        "Warning: Category already exists. Please ensure no duplicates are being added."
      );
      res.redirect("/admin/category");
    }
...
const hideCategory = async (req, res) => {
  try {
    ...
    const blockedCategory = await categorySchema.findByIdAndUpdate(categoryID, {
      isActive: false,
    });
...
const unHideCategory = async (req, res) => {
  try {
    ...
    const categoryID = req.params.id;
    // blocking the category with specific id
    const unBlockedCategory = await categorySchema.findByIdAndUpdate(
      categoryID,
      { isActive: true }
    );
    // redirect to category page
